...
Flags = True                                 
CLASSES = ("red","wolf","tiger","elephant","monkey","peacock")                             # 类别名（单类示例）
...
def main():
    ...
    ROI_X_MIN, ROI_X_MAX = 689, 1230
    ROI_Y_MIN, ROI_Y_MAX = 270, 810
            ...
            if boxes is not None:
                for box, cls, sc in zip(boxes, cls_ids, scores):
                    ...
                    if (ROI_X_MIN <= center_x <= ROI_X_MAX and ROI_Y_MIN <= center_y <= ROI_Y_MAX):
                        ...
                        cv2.putText(frame, f'{CLASSES[cls]} {sc:.2f}', (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                        ...
                        rospy.set_param("animal_name", CLASSES[cls])
                    
                        print(f"数量：{animal_num}，种类：{CLASSES[cls]}")
